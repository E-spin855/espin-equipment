// FILE: /api/project-mark-read.js
// PATH: /api/project-mark-read.js

import { kv } from "@vercel/kv";
import { sendBadgeOnlyPush } from "./_lib/push.js";

function clean(email) {
  return String(email || "").toLowerCase().trim();
}

function normalizeType(type) {
  const t = String(type || "").toLowerCase().trim();

  // Backward compatibility: old frontend used "tasks"
  if (t === "tasks") return "updates";

  if (t === "updates") return "updates";
  if (t === "images") return "images";

  return "";
}

async function recomputeUserBadge(email) {
  const cleanEmail = clean(email);

  const [updateKeys, imageKeys] = await Promise.all([
    kv.keys(`project:unread:*:${cleanEmail}`),
    kv.keys(`project:unread_images:*:${cleanEmail}`)
  ]);

  const keys = [...updateKeys, ...imageKeys];

  if (!keys.length) return 0;

  const values = await kv.mget(...keys);

  return values.reduce((sum, v) => {
    const n = Number(v);
    return sum + (Number.isFinite(n) ? n : 0);
  }, 0);
}

export default async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }

  const userEmail = clean(req.headers["x-user-email"]);
  const source = String(req.headers["x-clear-source"] || "").toLowerCase().trim();

  let body = {};

  try {
    body =
      typeof req.body === "string"
        ? JSON.parse(req.body || "{}")
        : req.body || {};
  } catch {
    return res.status(400).json({ error: "Invalid JSON" });
  }

  const projectId = String(body.projectId || "").trim();
  const type = normalizeType(body.type);

  console.log("MARK-READ", {
    userEmail,
    projectId,
    type,
    source
  });

  if (source !== "management") {
    console.log("BLOCKED: not management");
    return res.status(200).json({ ok: true });
  }

  if (!userEmail || !projectId) {
    return res.status(400).json({ error: "Missing parameters" });
  }

  if (type !== "updates" && type !== "images") {
    console.log("BLOCKED: invalid type");
    return res.status(200).json({ ok: true });
  }

  try {
    if (type === "updates") {
      await kv.del(`project:unread:${projectId}:${userEmail}`);
    }

    if (type === "images") {
      await kv.del(`project:unread_images:${projectId}:${userEmail}`);
    }

    const total = await recomputeUserBadge(userEmail);

    await Promise.all([
      kv.set(`app:badge:${userEmail}`, total),
      kv.set(`ios:badge:counter:global:${userEmail}`, total)
    ]);

    await sendBadgeOnlyPush(userEmail, total);

    return res.status(200).json({
      ok: true,
      badge: total
    });

  } catch (e) {
    console.error("mark-read error:", e);
    return res.status(500).json({ error: e.message });
  }
}