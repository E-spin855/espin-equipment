...
export default async function handler(req, res) {
  ...
  try {
    ...
    await Promise.all([
      kv.set(`app:badge:equipment:${email}`, badgeValue),
      kv.set(`ios:badge:counter:${email}`, badgeValue)
    ]);

    return res.status(200).json({ ok: true, badge: badgeValue });
  } catch (e) {
    console.error("equipment badge-refresh error:", e);
    return res.status(500).json({ ok: false, error: e.message || "Failed to refresh badge" });
  }
}